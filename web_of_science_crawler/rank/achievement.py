...
class Achievement:
    ...
    def __get_ccf_rank_dict(self, venue_: str, year: str) -> dict:
        """
        :param year 年份
        :param venue_可能有两种情况：
    ...
    def __get_jcr_rank_dict(self, venue_: str, year: str) -> dict:
        """
    ...
    def __get_cas_rank_dict(self, venue_:str, year: str) -> dict:
        """
        :param venue_只有一种情况：期刊。直接查表即可
    ...
    def get_rank_json(self):
        ...
        for row in self.paper_table.itertuples():
            ...
            jcr_rank_dict = self.__get_jcr_rank_dict(venue, pub_year)
            if jcr_rank_dict:
                ac['汤森路透分区'] = jcr_rank_dict

            cas_rank_dict = self.__get_cas_rank_dict(venue, pub_year)
            if cas_rank_dict:
            ...
            ccf_rank_dict = self.__get_ccf_rank_dict(get_index(venue), pub_year)
            if ccf_rank_dict:
    ...
    def get_score_json(self):
        ...
        result = {
            ...
            '汤森路透分区': {
                1: {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
                2: {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
                3: {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
                4: {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}}
            },
            '中科院分区': {
                1: {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
                2: {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
            ...
            'CCF': {
                'A': {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
                'B': {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}},
                'C': {'Count': 0, 'Score': {'Numerator': 0, 'Denominator': 1}}
            }
        }
        sum_score = Fraction()
        for row in self.paper_table.itertuples():
            contribution = row[3]
            score_denominator = 1
            if contribution == Contribution.FIRST_AUTHOR.name:
                pass
            elif contribution == Contribution.CORRESPONDING_AUTHOR.name:
                pass
            else:
                score_denominator = int(row[4])
            paper_score = Fraction(1, score_denominator)
            # sum_score += paper_score
            ...
            jcr_rank_dict = self.__get_jcr_rank_dict(venue, pub_year)
            if jcr_rank_dict:
                rank = int(jcr_rank_dict['Rank'])
                target = result['汤森路透分区'][rank]
                target['Count'] += 1
                new_score = Fraction(int(target['Score']['Numerator']), int(target['Score']['Denominator'])) + paper_score
                target['Score']['Numerator'], target['Score']['Denominator'] = new_score.numerator, new_score.denominator

            cas_rank_dict = self.__get_cas_rank_dict(venue, pub_year)
            if cas_rank_dict:
                rank = int(cas_rank_dict['Rank'])
                target = result['中科院分区'][rank]
                target['Count'] += 1
                new_score = Fraction(int(target['Score']['Numerator']),
                                     int(target['Score']['Denominator'])) + paper_score
                target['Score']['Numerator'], target['Score'][
                    'Denominator'] = new_score.numerator, new_score.denominator

            ccf_rank_dict = self.__get_ccf_rank_dict(get_index(venue), pub_year)
            if ccf_rank_dict:
                rank = ccf_rank_dict['Rank']
                target = result['CCF'][rank]
                target['Count'] += 1
                new_score = Fraction(int(target['Score']['Numerator']),
                                     int(target['Score']['Denominator'])) + paper_score
                target['Score']['Numerator'], target['Score'][
